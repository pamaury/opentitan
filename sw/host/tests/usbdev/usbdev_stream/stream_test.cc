...
//
// Usage:
//   stream [-v<bool>][-c<bool>][-r<bool>][-s<bool>] [<input port>[ <output
//   port>]]
//
    ...
    switch (dev->CurrentState()) {
      ...
      case USBDevice::StateSuspended:
        if (elapsed_time(start_time) >= kSuspendedInterval) {
          dev->Resume();
          dev->SetState(USBDevice::StateResuming);
...
int main(int argc, char *argv[]) {
  ...
  // Locate the USB device using Vendor and Product IDs.
