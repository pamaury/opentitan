...
static const dt_sysrst_ctrl_t *sysrst_ctrl_dt = &kDtSysrstCtrl[0];
...
static void pinmux_setup(void) {
  ...
  for (int i = 0; i < kNumMioInputs; ++i) {
    ...
    CHECK_DIF_OK(dif_pinmux_input_select(&pinmux, dt_pin_mio_periph_input(pin),
                                         kInputPads[i]));
  }
...
bool test_main(void) {
  ...
  CHECK_DIF_OK(dif_sysrst_ctrl_init_dt(sysrst_ctrl_dt, &sysrst_ctrl));
  if (kDeviceType == kDeviceSimDV) {
