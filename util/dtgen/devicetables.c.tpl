...
static const dt_instance_type_t instance_type[kDtInstanceIdCount] = {
% for module_name in module_types:
<%
    modules = [m for m in top["module"] if m["type"] == module_name]
%>\
%   for (dev_index, m) in enumerate(modules):
  [${snake_to_constant_name("dt_instance_id_" + m["name"])}] = ${snake_to_constant_name("dt_instance_type_" + m["type"])},
%   endfor
% endfor
};

static const dt_instance_type_t instance_index[kDtInstanceIdCount] = {
% for module_name in module_types:
<%
    modules = [m for m in top["module"] if m["type"] == module_name]
%>\
...
dt_instance_type_t dt_instance_type(dt_instance_id_t dev) {
  if (dev < kDtInstanceIdCount) {
    return instance_type[dev];
  }
  return kDtInstanceTypeUnknown;
}
