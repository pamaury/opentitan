...
class TopHelper:
    ...
    def _init_api(self):
        ...
        self.instance_type_enum = self._enum_type(Name([]), self.DT_INSTANCE_TYPE_NAME)
...
This can be `kDtPlicIrqIdNone` if the block is not connected to the PLIC."""
        ...
        if self.has_pins():
            self.inst_struct.add_field(
                name = Name(["pin"]),
                typename = ArrayType(
                    typename = TopHelper.DT_PIN_NAME,
                    length = self.pin_enum.name + Name(["count"]),
                ),
                docstring = "Description of each pin"
            )
